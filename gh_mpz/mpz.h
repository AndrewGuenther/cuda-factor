...
__device__ __host__ inline char* mpz_get_str(mpz_t *mpz, char *str, int bufsize) {
  ...
  for (i = mpz->capacity - 1; i >= 0; i--) {
    ...
    if (digit != 0 || print_zeroes) {
      ...
      if (!print_zeroes) {
        str_index += sprintf(str + str_index, "%u", digit);
      }
      else {
        str_index += sprintf(str + str_index, "%08u", digit);
      }
      print_zeroes = 1;
    }
  }
 ...
 *    gcd = a
 */
__device__ __inline__ void mpz_gcd(mpz_t *gcd, mpz_t *a, mpz_t *b) {
  int compare;
  mpz_t sub_tmp;
  mpz_init(&sub_tmp);

  while ((compare = mpz_compare(a, b)) != 0) {
     if (compare > 0) {
        mpz_sub(&sub_tmp, a, b);
        mpz_set(a, &sub_tmp);
     }
